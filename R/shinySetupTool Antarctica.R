...
make.group.object <- function(grp.file){
...
make.group.attributes <- function(prm.file, grp.vals){
...
make.general.params <- function(prm.file){
  prm <- readLines(prm.file) # read in the biological parameter file
  ...
  df.prms <- df.prms[c("Parameter","Units", "Value","Required", "Default", "Definition")] # name df columns
...
make.group.habitats <- function(prm.file, grp.object, habitat.types){
  prm        <- readLines(prm.file) # read in the biological parameter file
...
make.group.distributions <- function(prm.file, Code, numboxes){
  prm   <- readLines(prm.file) # read in the biological parameter file
  Code <- sort(Code)
  codes <- length(Code)
...
make.prey.data <- function(prm.file, Code) {
  prm <- readLines(prm.file) # read in the biological parameter file
  Code <- sort(Code)
  codes <- length(Code)
...
make.refuge.data <- function(grp.vals, gen.prm, grp.att) {
...
make.setup.object <- function(bgm.file, grp.file, prm.file) {
  cat("-- Extracting map data\n")
  map.objects <- make.map.object(bgm.file)
  numboxes <- map.objects$numboxes
  
  cat("-- Reading group summaries\n")
  grp.object <- make.group.object(grp.file)
  grp.vals <- grp.object$grp.vals
  Code <- grp.vals$Code
  habitat.types <- grp.object$habitat.types
  
  cat("-- Extracting general parameters\n")
  gen.prm <- make.general.params(prm.file)
  
  cat("-- Extracting group parameters (this may take a few minutes)\n")
  grp.att <- make.group.attributes(prm.file, grp.vals)
  
  cat("-- Extracting habitat parameters\n")
  grp.hab <- make.group.habitats(prm.file, grp.vals, habitat.types) # three data frames
  
  cat("-- Extracting horizontal distributions (this may take a few minutes)\n")
  grp.dist <- make.group.distributions(prm.file, Code, numboxes)
  
  cat("-- Extracting prey availability (this may take a few minutes)\n")
  prey.data <- make.prey.data(prm.file, Code)
  
  cat("-- Extracting refuge parameterisation\n")
  refuge.data <- make.refuge.data(grp.vals, gen.prm, grp.att)
